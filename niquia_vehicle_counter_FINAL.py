...
# ══════════════════════════════════════════════════════════════════════════════
#  SECTION 7 — WHITE LINE DETECTOR (Niquía-specific tuning)
# ══════════════════════════════════════════════════════════════════════════════
#
#  What we know from the 4 actual camera images:
#
#  CAM1 (NS, empty road): Strong solid white lines, deep perspective,
#    camera mounted top-left. Lines converge steeply to upper-centre.
#
#  CAM2 (NS, busy): Dashed white centre line + solid outer edges.
#    Moderate glare top-right. 3 lanes clearly visible.
#
#  CAM3 (SN, busy evening): Strong glare top-right, solid outer lane lines,
#    dashed inner line. Lower camera angle shows truck bodies prominently.
#
#  CAM4 (SN, wider view): Lower mount, wider angle, includes hard shoulder.
#    Solid line between lane and shoulder clearly visible.
#
#  SHARED CHARACTERISTICS:
#    - Dark asphalt = excellent contrast for white lines
#    - Near-perspective: lines are diagonal in frame (not vertical)
#    - Vanishing point ~30-40% from top, centre of frame
#    - All cameras show both solid and dashed white markings
# ──────────────────────────────────────────────────────────────────────────────

@dataclass
class LaneBoundary:
    """Single lane boundary — described as two points top/bottom of ROI."""
    x_top:    float
    x_bottom: float
    y_top:    int
    y_bottom: int
...
class WhiteLineDetector:
    """
    Detects white/yellow painted lane markings from CCTV road footage.

    Pipeline:
      1. Glare suppression: overexposed pixels → mid-grey (neutralises sun glare)
      2. HLS colour space → isolate white + yellow lane pixels
      3. Morphological CLOSE → connect dashed markings into solid segments
      4. Morphological OPEN  → remove small noise blobs
      5. Canny edge detection on original grey, masked to lane-marking pixels
      6. Trapezoid ROI mask (road area only, matching Niquía perspective)
      7. Probabilistic Hough line transform
      8. Filter: keep only lines with significant diagonal angle
         (rejects horizontal cracks, road signs, car bonnets)
      9. Fit a line to each segment; compute x-intercept at reference y
     10. K-means cluster into exactly n_lanes+1 boundary positions
     11. Temporal smoothing over last 15 frames → eliminates frame-to-frame flicker
    """

    def __init__(self, H: int, W: int,
                 roi_top_frac: float=0.28, roi_bottom_frac: float=0.97):
        self.H=H; self.W=W
    ...
    def _suppress_glare(self, frame: np.ndarray) -> np.ndarray:
        ...
        _,gm=cv2.threshold(gray,230,255,cv2.THRESH_BINARY)
        k=cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(21,21))
        gm=cv2.dilate(gm,k)
        out=frame.copy(); out[gm==255]=[80,80,80]
        return out

    # ── 2-4. White/yellow mask ────────────────────────────────────────────────
    def _white_mask(self, frame: np.ndarray) -> np.ndarray:
        ng =self._suppress_glare(frame)
        hls=cv2.cvtColor(ng,cv2.COLOR_BGR2HLS)
        # White: high lightness (>155), low saturation (<50)
        white =cv2.inRange(hls,np.array([0,155,0],np.uint8),
                               np.array([180,255,50],np.uint8))
        # Yellow: hue 15-38, moderate+ lightness, high saturation
        yellow=cv2.inRange(hls,np.array([15,80,90],np.uint8),
                               np.array([38,220,255],np.uint8))
        combined=cv2.bitwise_or(white,yellow)
        # Connect dashes (tall kernel closes vertical gaps)
        k1=cv2.getStructuringElement(cv2.MORPH_RECT,(3,18))
    ...
    # ── 6. Perspective trapezoid ROI ──────────────────────────────────────────
    def _roi_mask(self, mask: np.ndarray) -> np.ndarray:
        """
        Trapezoid tuned to Niquía cameras:
          - Narrow at top (near vanishing point, ~28-30% height)
          - Wide at bottom (full road width visible near camera)
          - Excludes sky, grass verge, roadside structures
        """
        out=np.zeros_like(mask)
        pts=np.array([
            [int(self.W*0.04), self.roi_bottom],  # bottom-left (road edge)
            [int(self.W*0.28), self.roi_top],      # top-left
            [int(self.W*0.92), self.roi_top],      # top-right
            [int(self.W*0.99), self.roi_bottom],   # bottom-right
        ],dtype=np.int32)
        cv2.fillPoly(out,[pts],255)
        return cv2.bitwise_and(mask,out)

    # ── 7-8. Hough + angle filter ─────────────────────────────────────────────
    def _detect_lines(self, frame: np.ndarray) -> List[Tuple[int,int,int,int]]:
        white=self._white_mask(frame)
        roi  =self._roi_mask(white)
        gray =cv2.cvtColor(frame,cv2.COLOR_BGR2GRAY)
        ...
        lines=cv2.HoughLinesP(edges,1,np.pi/180,
                               threshold=25,
                               minLineLength=int(self.H*0.05),
                               maxLineGap  =int(self.H*0.08))
        ...
        for l in lines:
            ...
            # Keep lines with meaningful diagonal angle
            # |slope|>0.15 rejects horizontal noise (road cracks, horizon artifacts)
            if abs(dy/dx)<0.15: continue
            valid.append((x1,y1,x2,y2))
        return valid

    # ── 9-10. Cluster into lane boundaries ───────────────────────────────────
    def _cluster(self, lines: List, n_lanes: int) -> List[Optional[Tuple[float,float]]]:
        """
        Fit a line to each segment.
        Cluster by x-intercept at mid-ROI into n_lanes+1 boundaries.
        Returns list of (slope, intercept) tuples (or None if missing).
        """
        if not lines:
            return [None]*(n_lanes+1)
        ...
        for x1,y1,x2,y2 in lines:
            ...
            vx=float(vx[0]); vy=float(vy[0]); x0=float(x0[0]); y0=float(y0[0])
            if abs(vx)<1e-6: continue
            slope=vy/vx
            intercept=y0-slope*x0
            xm=(mid_y-intercept)/slope if abs(slope)>1e-6 else x0
            fitted.append((slope,intercept,xm))

        if not fitted: return [None]*(n_lanes+1)
        fitted.sort(key=lambda f:f[2])

        # Merge lines within 8% frame width (same physical marking)
        thresh=self.W*0.08
        merged: List[List]=[]
        for s,i,xm in fitted:
            if merged and abs(xm-merged[-1][-1][2])<thresh:
                merged[-1].append((s,i,xm))
            else:
                merged.append([(s,i,xm)])

        result=[]
        for cluster in merged:
            result.append((float(np.mean([c[0] for c in cluster])),
                           float(np.mean([c[1] for c in cluster]))))

        while len(result)<n_lanes+1: result.append(None)
        return result[:n_lanes+1]

    # ── 11. Temporal smoothing ────────────────────────────────────────────────
    def _smooth(self, new_fits: List) -> List:
        self._history.append(new_fits)
        if len(self._history)>self._max_hist: self._history.pop(0)
        out=[]
        for i in range(len(new_fits)):
            vals=[h[i] for h in self._history if i<len(h) and h[i] is not None]
            if vals:
    ...
    # ── Public API ────────────────────────────────────────────────────────────
    def detect(self, frame: np.ndarray, n_lanes: int
               ) -> Optional[List[Optional[LaneBoundary]]]:
        lines =self._detect_lines(frame)
        fits  =self._cluster(lines,n_lanes)
        smooth=self._smooth(fits)

        def to_b(f):
            if f is None: return None
            slope,intercept=f
            if abs(slope)<1e-6: return None
            def xaty(y): return (y-intercept)/slope
            return LaneBoundary(
                x_top   =xaty(self.roi_top),
                x_bottom=xaty(self.roi_bottom),
                y_top   =self.roi_top,
                y_bottom=self.roi_bottom,
                detected=True)

        boundaries=[to_b(f) for f in smooth]
        return boundaries if any(b is not None for b in boundaries) else None


# ══════════════════════════════════════════════════════════════════════════════
#  SECTION 8 — LANE MANAGER
# ══════════════════════════════════════════════════════════════════════════════
...
class LaneManager:
    """
    Manages lane boundaries with perspective-aware white-line detection.

    For Niquía cameras the road occupies approx:
      Left edge:  8% of frame width (top and bottom)
      Right edge: 92% at top, 97% at bottom (perspective widening)

    When white-line detection finds:
      n_lanes+1 lines → assign all boundaries directly
      2 lines only    → treat as outer edges, interpolate inner dividers
      3 lines for 3-lane road → assign left 2 and extrapolate right edge
    """

    def __init__(self, W: int, H: int, n_lanes: int,
                 direction: str="NS", mode: str="auto",
                 roi_top_frac: float=0.28):
        self.W=W; self.H=H; self.n_lanes=n_lanes
        self.direction=direction; self.mode=mode
        self.roi_top=int(H*roi_top_frac)
        self.detector=WhiteLineDetector(H,W,roi_top_frac)
        self.lanes: List[Lane]=[]
        self._frame_n=0; self._detect_every=8
        self._build_uniform()

    def _build_uniform(self):
        """Perspective-correct fallback with uniform lane division."""
        rl_t=self.W*0.08;  rr_t=self.W*0.92
        rl_b=self.W*0.08;  rr_b=self.W*0.97
        rw_t=rr_t-rl_t;   rw_b=rr_b-rl_b
        self.lanes=[]
        for i in range(self.n_lanes):
            lt=rl_t+i*rw_t/self.n_lanes;     rt=rl_t+(i+1)*rw_t/self.n_lanes
            lb=rl_b+i*rw_b/self.n_lanes;     rb=rl_b+(i+1)*rw_b/self.n_lanes
            left =LaneBoundary(x_top=lt,x_bottom=lb,y_top=self.roi_top,y_bottom=self.H)
            right=LaneBoundary(x_top=rt,x_bottom=rb,y_top=self.roi_top,y_bottom=self.H)
            col=_LANE_COLORS[i%len(_LANE_COLORS)]
            self.lanes.append(Lane(number=i+1,left=left,right=right,
                                   color=col,label=f"Carril {i+1}"))

    def _update_from_detection(self, boundaries: List[Optional[LaneBoundary]]):
        detected=[(i,b) for i,b in enumerate(boundaries) if b is not None]
        if len(detected)<2: return
        detected.sort(key=lambda ib: ib[1].x_bottom)
        good=[b for _,b in detected]

        if len(good)>=self.n_lanes+1:
            for i in range(self.n_lanes):
                self.lanes[i].left=good[i]; self.lanes[i].right=good[i+1]
        elif len(good)==2:
            ol=good[0]; or_=good[-1]
            for i in range(self.n_lanes):
                tl=i/self.n_lanes; tr=(i+1)/self.n_lanes
                self.lanes[i].left=LaneBoundary(
                    x_top   =ol.x_top   +tl*(or_.x_top   -ol.x_top),
                    x_bottom=ol.x_bottom+tl*(or_.x_bottom-ol.x_bottom),
                    y_top=ol.y_top,y_bottom=ol.y_bottom,detected=True)
                self.lanes[i].right=LaneBoundary(
                    x_top   =ol.x_top   +tr*(or_.x_top   -ol.x_top),
                    x_bottom=ol.x_bottom+tr*(or_.x_bottom-ol.x_bottom),
                    y_top=ol.y_top,y_bottom=ol.y_bottom,detected=True)
        elif len(good)==3 and self.n_lanes==3:
            dx_t=good[2].x_top   -good[1].x_top
            dx_b=good[2].x_bottom-good[1].x_bottom
            self.lanes[0].left=good[0]; self.lanes[0].right=good[1]
            self.lanes[1].left=good[1]; self.lanes[1].right=good[2]
            self.lanes[2].left=good[2]
            self.lanes[2].right=LaneBoundary(
                x_top   =good[2].x_top   +dx_t,
                x_bottom=good[2].x_bottom+dx_b,
                y_top=good[2].y_top,y_bottom=good[2].y_bottom,detected=False)

    def update(self, frame: np.ndarray):
        self._frame_n+=1
        if self.mode=="auto" and self._frame_n%self._detect_every==0:
            result=self.detector.detect(frame,self.n_lanes)
            if result: self._update_from_detection(result)

    def get_lane(self, cx: int, cy: int=0) -> int:
        """Return 1-based lane number for vehicle centroid (cx,cy)."""
        if cy<=0: cy=int(self.H*0.65)
        for lane in self.lanes:
            if lane.contains(cx,cy): return lane.number
        ref_y=int(self.H*0.65)
    ...
    def draw(self, frame: np.ndarray, lane_counts: Optional[dict]=None):
        H=frame.shape[0]; overlay=frame.copy()
        for lane in self.lanes:
            pts=np.array([
            ...
            dim=tuple(max(0,int(c*0.18)) for c in lane.color)
        ...
        for i,lane in enumerate(self.lanes):
            for side in ("left","right"):
                ...
                is_outer=(i==0 and side=="left") or (i==self.n_lanes-1 and side=="right")
                xt=b.x_at(self.roi_top); xb=b.x_at(H)
                col=(255,255,255) if b.detected else (160,160,80)
                thick=2 if is_outer else 1
                cv2.line(frame,(xt,self.roi_top),(xb,H),col,thick,cv2.LINE_AA)

            ref_y=int(H*0.25); cx_lbl=lane.center_x(ref_y)
            total=sum(lane_counts.get(lane.number,{}).values()) if lane_counts else 0
            cv2.putText(frame,f"L{lane.number}:{total}",
                        (cx_lbl-22,ref_y),cv2.FONT_HERSHEY_SIMPLEX,
                        0.52,lane.color,1,cv2.LINE_AA)
...
def run(
    ...
    lane_mgr  = LaneManager(W,H,n_lanes,direction,mode=lane_mode,roi_top_frac=roi_top_frac)
