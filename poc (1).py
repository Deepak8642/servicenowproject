...
class WhiteLineDetector:
    """
    Perspective-aware lane marking detector tuned for angled CCTV highway cameras
    (like Niquía toll station footage).

    Improvements over basic version:
    - Glare suppression: masks over-exposed regions before processing
    - Adaptive HLS thresholds for varying lighting
    - Wider ROI trapezoid matching the actual road perspective
    - Multi-line detection returning all candidate boundaries, not just left/right pair
    - Temporal smoothing over 15 frames for stability
    - Works for 2 and 3 lane configurations
    """

    def __init__(self, frame_h: int, frame_w: int,
                 roi_top_frac: float = 0.30, roi_bottom_frac: float = 0.98):
        ...
        self._max_history = 15   # more frames → smoother lines

    def _suppress_glare(self, frame: np.ndarray) -> np.ndarray:
        """Darken blown-out (glare) regions so they don't confuse line detection."""
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        glare_mask = cv2.threshold(gray, 230, 255, cv2.THRESH_BINARY)[1]
        # dilate glare region slightly
        k = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (15, 15))
        glare_mask = cv2.dilate(glare_mask, k)
        result = frame.copy()
        result[glare_mask == 255] = [80, 80, 80]   # replace with mid-gray
        return result

    def _extract_white_mask(self, frame: np.ndarray) -> np.ndarray:
        """Extract white AND yellow lane markings with adaptive thresholds."""
        frame_ng = self._suppress_glare(frame)
        hls = cv2.cvtColor(frame_ng, cv2.COLOR_BGR2HLS)
        # White lines: high lightness, low saturation
        white = cv2.inRange(hls,
                            np.array([0,   160,  0], np.uint8),
                            np.array([180, 255, 45], np.uint8))
        # Yellow lines
        yellow = cv2.inRange(hls,
                             np.array([15,  80, 90], np.uint8),
    ...
    def _apply_roi(self, mask: np.ndarray) -> np.ndarray:
        """
        Wider trapezoid ROI matching the angled CCTV perspective
        seen in the Niquía footage (camera mounted ~45° above road).
        """
        roi = np.zeros_like(mask)
        # Trapezoid: narrow at top (vanishing point), wide at bottom
        pts = np.array([
            [int(self.W * 0.08), self.roi_bottom],
            [int(self.W * 0.35), self.roi_top],
            [int(self.W * 0.92), self.roi_top],
            [int(self.W * 0.97), self.roi_bottom],
        ], dtype=np.int32)
        cv2.fillPoly(roi, [pts], 255)
        return cv2.bitwise_and(mask, roi)

    def _hough_lines(self, edges: np.ndarray) -> List:
        """
        Detect all lane lines, returning them sorted left-to-right by
        their x-position at the bottom of the ROI.
        """
        ...
        for line in lines:
            ...
            if x2 == x1:
                continue
            slope = (y2 - y1) / (x2 - x1)
            # Accept lines with any significant diagonal angle
            # (handles both left-leaning and right-leaning lane lines
            #  from angled cameras)
            if abs(slope) < 0.20:
                continue
    ...
    def _cluster_lines(self, lines: List, n_expected: int) -> List[Optional[Tuple[float,float]]]:
        """
        Cluster detected lines into n_expected+1 lane boundaries
        (need one more boundary than lanes) based on their x-intercept
        at the middle of the ROI. Returns (slope, intercept) per cluster.
        """
        if not lines:
            return [None] * (n_expected + 1)
        ...
        for x1, y1, x2, y2 in lines:
            ...
            if abs(float(vx[0])) < 1e-6:
                continue
            slope = float(vy[0]) / float(vx[0])
            intercept = float(y0[0]) - slope * float(x0[0])
            x_mid = (mid_y - intercept) / slope if abs(slope) > 1e-6 else float(x0[0])
            fitted.append((slope, intercept, x_mid))

        if not fitted:
            return [None] * (n_expected + 1)

        # Sort by x-position at mid-y
        fitted.sort(key=lambda f: f[2])

        # Merge lines that are very close (same physical lane boundary)
        merged: List[List] = []
        threshold = self.W * 0.08   # 8% of frame width
        for slope, intercept, x_mid in fitted:
            if merged and abs(x_mid - merged[-1][-1][2]) < threshold:
                merged[-1].append((slope, intercept, x_mid))
            else:
                merged.append([(slope, intercept, x_mid)])

        # Average each cluster
        result = []
        for cluster in merged:
            avg_slope = float(np.mean([c[0] for c in cluster]))
            avg_inter = float(np.mean([c[1] for c in cluster]))
            result.append((avg_slope, avg_inter))

        # Pad or trim to n_expected+1 boundaries
        n_target = n_expected + 1
        while len(result) < n_target:
            result.append(None)
        return result[:n_target]

    def _smooth_boundaries(self, new_fits: List[Optional[Tuple[float,float]]]) -> List[Optional[Tuple[float,float]]]:
        """Temporal smoothing: average over last _max_history frames."""
    ...
    def _fit_to_boundary(self, slope: float, intercept: float) -> LaneBoundary:
        def x_at_y(y):
            return (y - intercept) / slope if abs(slope) > 1e-6 else intercept
        return LaneBoundary(
            x_top    = x_at_y(self.roi_top),
            x_bottom = x_at_y(self.roi_bottom),
            y_top    = self.roi_top,
            y_bottom = self.roi_bottom,
            detected = True,
        )

    def detect(self, frame: np.ndarray, n_lanes: int = 2) -> Optional[List[LaneBoundary]]:
        """
        Returns list of LaneBoundary (length n_lanes+1) or None if detection failed.
        """
        white  = self._extract_white_mask(frame)
        roi    = self._apply_roi(white)
        edges  = cv2.Canny(roi, 40, 120)
        lines  = self._hough_lines(edges)
        fits   = self._cluster_lines(lines, n_lanes)
        smooth = self._smooth_boundaries(fits)

        boundaries = [self._fit_to_boundary(*f) if f else None for f in smooth]
        if any(b is not None for b in boundaries):
            return boundaries
        return None
...
class LaneManager:
    """
    Improved lane manager for angled CCTV cameras (Niquía toll footage).

    Key improvements:
    - Perspective-correct uniform fallback: lane widths taper toward vanishing point
    - Handles 2-lane and 3-lane road sections correctly
    - Updates from detection every 10 frames (faster response)
    - Smarter boundary interpolation when only 2 outer lines detected
    - Draws perspective-correct lane overlays with visible colours
    """

    _COLORS = [
        (255, 255, 120),   # yellow
        (120, 220, 255),   # light blue
        (120, 255, 160),   # light green
        (255, 180, 120),   # orange
        (220, 120, 255),   # purple
    ]
    ...
    def __init__(self, frame_w: int, frame_h: int, n_lanes: int,
                 ...
                 manual_zones: Optional[List] = None, roi_top_frac: float = 0.30):
        ...
        self._frame_count = 0
        self._detect_every = 10   # detect more frequently

        if mode == "manual" and manual_zones:
            self._build_from_zones(manual_zones)
        else:
            self._build_perspective_uniform()

    def _build_perspective_uniform(self):
        """
        Build lanes with perspective-correct widths.
        At the top (far end) of the road, lanes are narrower.
        At the bottom (near camera), lanes are wider.
        This matches the angled overhead CCTV view.
        """
        # Road boundaries in the frame (estimated from Niquía footage)
        # Left edge: starts at ~8% width at top, ~10% at bottom
        # Right edge: starts at ~90% at top, ~95% at bottom
        road_left_top    = self.W * 0.08
        road_left_bottom = self.W * 0.10
        road_right_top   = self.W * 0.90
        road_right_bottom= self.W * 0.95

        road_width_top    = road_right_top    - road_left_top
        road_width_bottom = road_right_bottom - road_left_bottom

        self.lanes = []
        for i in range(self.n_lanes):
            lt = road_left_top    + i       * road_width_top    / self.n_lanes
            rt = road_left_top    + (i + 1) * road_width_top    / self.n_lanes
            lb = road_left_bottom + i       * road_width_bottom / self.n_lanes
            rb = road_left_bottom + (i + 1) * road_width_bottom / self.n_lanes

            left  = LaneBoundary(x_top=lt, x_bottom=lb,
                                  y_top=self.roi_top, y_bottom=self.H)
            right = LaneBoundary(x_top=rt, x_bottom=rb,
                                  y_top=self.roi_top, y_bottom=self.H)
    ...
    def _update_from_detection(self, boundaries: List[Optional[LaneBoundary]]):
        """
        Update lane boundaries from detected lines.
        boundaries is a list of n_lanes+1 items (one per lane edge).
        Falls back gracefully when some boundaries are missing.
        """
        detected = [(i, b) for i, b in enumerate(boundaries) if b is not None]
        if len(detected) < 2:
            return   # not enough info to update

        # Sort by x-position at bottom of frame
        detected.sort(key=lambda ib: ib[1].x_bottom)
        good_bounds = [b for _, b in detected]

        if len(good_bounds) >= self.n_lanes + 1:
            # Enough boundaries: assign directly
            for i in range(self.n_lanes):
                self.lanes[i].left  = good_bounds[i]
                self.lanes[i].right = good_bounds[i + 1]
        elif len(good_bounds) == 2:
            # Only outer edges found: interpolate inner lane dividers
            ol = good_bounds[0]; or_ = good_bounds[-1]
            for i in range(self.n_lanes):
                t_l = i       / self.n_lanes
                t_r = (i + 1) / self.n_lanes
                self.lanes[i].left  = LaneBoundary(
                    x_top    = ol.x_top    + t_l * (or_.x_top    - ol.x_top),
                    x_bottom = ol.x_bottom + t_l * (or_.x_bottom - ol.x_bottom),
                    y_top=ol.y_top, y_bottom=ol.y_bottom, detected=True)
                self.lanes[i].right = LaneBoundary(
                    x_top    = ol.x_top    + t_r * (or_.x_top    - ol.x_top),
                    x_bottom = ol.x_bottom + t_r * (or_.x_bottom - ol.x_bottom),
                    y_top=ol.y_top, y_bottom=ol.y_bottom, detected=True)
        # else: only 1 boundary detected, ignore to avoid wild jumps
    ...
    def update(self, frame: np.ndarray):
        ...
        if self.mode == "auto" and self._frame_count % self._detect_every == 0:
            ...
            if boundaries:
                self._update_from_detection(boundaries)

    def get_lane(self, cx: int, cy: int = 0) -> int:
        """Return lane number (1-based) for a vehicle centroid at (cx, cy)."""
        # Use a representative y near the counting zone if cy==0
        if cy == 0:
            cy = int(self.H * 0.65)
        for lane in self.lanes:
            if lane.contains_cx(cx, cy):
                return lane.number
        # Fallback: nearest lane centre
        mid_y = int(self.H * 0.65)
    ...
    def draw(self, frame: np.ndarray, lane_counts: Optional[dict] = None):
        ...
        for i, lane in enumerate(self.lanes):
            ...
            color_dim = tuple(max(0, int(c * 0.18)) for c in lane.color)
        ...
        for i, lane in enumerate(self.lanes):
            is_leftmost  = (i == 0)
            is_rightmost = (i == self.n_lanes - 1)
            # Solid outer edges
            if is_leftmost:
                self._draw_solid_line(frame, lane.left,  lane.color, 3)
            if is_rightmost:
                self._draw_solid_line(frame, lane.right, lane.color, 3)
            # Dashed inner dividers
            if not is_leftmost:
                self._draw_dashed_line(frame, lane.left, lane.color, 2, 28, 18)
            if not is_rightmost:
                self._draw_dashed_line(frame, lane.right, lane.color, 2, 28, 18)
    ...
    def _draw_lane_badge(frame, lane: Lane, cx: int, cy: int, count: int):
        ...
        bg = tuple(max(0, int(c * 0.4)) for c in lane.color)
        ...
        cv2.putText(frame, sub, (x0 + (bw - sw) // 2, y0 + bh - pad + 2),
                    font, 0.38, (200, 255, 200), 1, cv2.LINE_AA)
...
def run(source,
    ...
    if count_line_fracs is None:
        count_line_fracs = [0.42, 0.57, 0.72]   # tuned for angled overhead CCTV
    ...
    while True:
        ...
        # Track
        active = tracker.step(dets, lambda cx: lane_mgr.get_lane(cx, cy=int(H * 0.65)))
