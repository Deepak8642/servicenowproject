...
class ScanlineWhiteDetector:
    ...
    def detect_boundary_xs(self, frame: np.ndarray,
        ...
        pts = np.array([
            [int(self.W * 0.02), self.roi_bottom],
            [int(self.W * 0.22), self.roi_top],
            [int(self.W * 0.78), self.roi_top],
            [int(self.W * 0.98), self.roi_bottom],
        ], dtype=np.int32)
...
class LaneManager:
    ...
    def __init__(self, W: int, H: int, n_lanes: int,
        ...
        self.detector = ScanlineWhiteDetector(H, W, roi_top_frac, 0.92)
    ...
    def _xs_bottom_to_lanes(self, xs_bottom: List[float],
        ...
        """
        n = self.n_lanes
        vp_x = self.W * 0.50  # vanishing point x (centre of frame)
    ...
    def _build_uniform(self):
        ...
        """
        road_left  = self.W * 0.08
        road_right = self.W * 0.96
    ...
    def _try_track_calibration(self) -> bool:
        ...
        xs_bound[0]  = max(xs_bound[0],  self.W * 0.02)
        xs_bound[-1] = min(xs_bound[-1], self.W * 0.98)
